...
function parseJsonConvertingSnakeToCamel(str) {
  return JSON.parse(str, function (key, value) {
    if (key === "org_id") {
      ...
      this.orgHelper = getOrgHelper$1(value);
      this.accessHelper = getAccessHelper$1(value);
    } else if (key === "user_id") {
      this.userId = value;
...
const AuthProvider = props => {
  ...
  const [loggedInChangeCounter, setLoggedInChangeCounter] = React.useState(0); // Create a client and register an observer that triggers when the user logs in or out
  ...
  }, [client]); // Refresh the token when the user has logged in or out
  ...
  }, [client, loggedInChangeCounter]); // Watchdog timer to make sure that if we hit the expiration we get rid of the token.
  ...
  const redirectToSetupSAMLPage = React.useCallback(client.redirectToSetupSAMLPage, []);
  const getLoginPageUrl = React.useCallback(client.getLoginPageUrl, []);
  const getSignupPageUrl = React.useCallback(client.getSignupPageUrl, []);
  const getAccountPageUrl = React.useCallback(client.getAccountPageUrl, []);
  const getOrgPageUrl = React.useCallback(client.getOrgPageUrl, []);
  const getCreateOrgPageUrl = React.useCallback(client.getCreateOrgPageUrl, []);
  const getSetupSAMLPageUrl = React.useCallback(client.getSetupSAMLPageUrl, []);
...
const RequiredAuthProvider = props => {
  ...
  return /*#__PURE__*/React__default["default"].createElement(AuthProvider, sharedProps, /*#__PURE__*/React__default["default"].createElement(WrappedComponent, null));
};
